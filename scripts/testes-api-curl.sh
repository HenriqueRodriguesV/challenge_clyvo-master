...
curl -s -X POST "$BASE_URL/responsaveis" \
  ...
  -d '{"nome":"Beatriz Lima","email":"beatriz.lima@email.com","cpf":"56789012345","dataNascimento":"1998-06-12"}' | jq . || true
...
curl -s -X POST "$BASE_URL/pets" \
  -H "Content-Type: application/json" \
  -d '{"nome":"Luna","descricao":"Pet com monitoramento preventivo por perfil comportamental","raca":"SRD","dataNascimento":"2022-01-20","responsavelId":1}' | jq . || true

echo "5) Atualizar pet 1"
curl -s -X PUT "$BASE_URL/pets/1" \
  -H "Content-Type: application/json" \
  -d '{"nome":"Thor","descricao":"Cachorro acompanhado pela triagem inteligente da Clyvo","raca":"Golden Retriever","dataNascimento":"2021-03-15","responsavelId":1}' | jq . || true
